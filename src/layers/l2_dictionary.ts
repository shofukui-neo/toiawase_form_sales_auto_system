...
export const ROLE_RULES: RoleRule[] = [
  ...
  },
  {
    ...
    keywords: [
      '氏名',
      'お名前',
      // 「お」が付かない「名前」だけの見出しが辞書から漏れており、
      // 名前欄が対応付かないまま「必須なのに未マッピング」として
      // 企業ごと除外されていた（unfillable_required の主要因のひとつ）。
      '名前',
      'ご担当者',
      'ご氏名',
      '担当者',
  ...
  {
    // FAX を必須にしているフォームが実在する。当社は実際に FAX 番号を
    // 持っているので、埋められない欄として企業を捨てる理由がない。
    // 電話より先に判定されないよう、電話系の語は除外する。
    role: 'fax',
    keywords: ['fax', 'ｆａｘ', 'ファクシミリ', 'ファックス'],
    weight: 0.92,
  },
  {
    role: 'subject',
