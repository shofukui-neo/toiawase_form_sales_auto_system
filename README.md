...
`SENDER_KANA` に加えて `SENDER_KANA_SEI` / `SENDER_KANA_MEI` も設定しておくこと —
フリガナが**セイ/メイに分かれている**フォームは、分割値が無いと必須バリデーションで弾かれる。
...
実送信できる。※最終送信は**送信可能時間帯**（`.env` の `SEND_WINDOW_START`〜`END`）かつ日次上限内で
のみ実行される（ペーシング §9）。既定は `0`〜`24` の**24時間送信** — フォーム送信は相手の受信箱を
夜中に鳴らさないので、夜間・営業時間外でも止めない。絞りたいときだけ窓を狭める（例: 9〜19）。
...
（既定3）で並列に処理する。送信（L4 Execute）も同様に `SEND_CONCURRENCY`（既定3）で並列化される。
同時に生きる Chromium の総数は `BROWSER_MAX_CONCURRENCY`（既定 = `INTAKE_CONCURRENCY` +
`SEND_CONCURRENCY`）で頭を押さえる。
...
- **並列送信** — 送信ワーカーを `SEND_CONCURRENCY`（既定3）本立て、候補キューを共有して複数社を
  同時に処理する。1社の送信は 30〜90 秒（大半がネットワーク待ち）かかるため、直列だと送信間隔より
  実処理時間が支配的になっていた。送信間隔はワーカーごとに独立して効くので、実効スループットは
  概ね並列数倍になる。同じ企業を 2 本のワーカーが掴まないよう候補は掴んだ時点で予約され、
  送信直前の readiness 再評価と `already_sent` 抑制が二重送信の最終防壁として残る。
- 送信間隔・日次上限・送信可能時間帯（§9 pacing）は従来どおり全て通る。ただし**送信可能時間帯は
  既定で 24 時間**（`SEND_WINDOW_START=0` / `SEND_WINDOW_END=24`）なので、夜間・営業時間外でも
  送信は止まらない。窓を狭めた場合や上限到達時は、ワーカーが**待機**し窓が開いたら自動で再開する
  （「■ 中断」でいつでも止められる）。
- 送信直前にもう一度同じ判定をかけるので、判定後に編集・却下・抑制された企業は送られない。

API: `GET /api/sendable`（送信可能／保留の一覧と理由）/ `POST /api/execute-all`（`{follow:true}` で随時送信、
`{concurrency:N}` でそのランだけ並列数を上書き）/
